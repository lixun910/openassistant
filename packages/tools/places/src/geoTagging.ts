...
const geotaggingParameters = z.object({
  ll: z
    .string()
    .describe(
...
export type GeotaggingFunctionArgs = z.infer<typeof geotaggingParameters>;
 ...
 */
export const geotagging: OpenAssistantTool<
  typeof geotaggingParameters,
  GeotaggingLlmResult,
...
> = {
  ...
  execute: async (args, options): Promise<ExecuteGeotaggingResult> => {
    ...
    try {
      ...
      const candidatesData: GeotaggingCandidateData[] = data.candidates.map(
        (candidate) => ({
