  ...
  if (isStringArray) {
    ...
    const displayBins = hasOthers 
      ? [
          ...
          {
            value: "Others",
            count: sortedBins.slice(MAX_UNIQUE_VALUES).reduce((sum, bin) => sum + bin.count, 0),
            indices: sortedBins.slice(MAX_UNIQUE_VALUES).flatMap(bin => bin.indices),
          }
        ]
      : sortedBins;
    ...
    const histogramData: StringHistogramDataProps[] = displayBins.map((bin, index) => ({
      bin: index,
      binStart: bin.value,
      binEnd: bin.value,
    }));
