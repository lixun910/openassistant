 ...
 */
type GetValues = (datasetName: string, xVariableName: string, yVariableName: string) => Promise<{ x: number[], y: number[] }>;
 ...
 */
type OnSelectedCallback = (datasetName: string, selectedIndices: number[]) => void;
 ...
 */
export type ScatterplotFunctionContextValues = ScatterplotFunctionContext[keyof ScatterplotFunctionContext];
...
export function scatterplotFunctionDefinition(
  ...
  return {
    ...
    properties: {
      datasetName: {
        type: 'string',
        description: 'The name of the dataset to create a scatterplot. If not provided, use the first dataset or ask user to select or upload a dataset.',
      },
      xVariableName: {
        type: 'string',
