...
export function ScatterRegressionPlot({data, filteredIndexes}: ScatterRegressionPlotProps) {
  ...
  const option = useMemo(() => {
    if (!data) return {};
    try {
      const { xData, yData, xVariableName, yVariableName, showRegressionLine, showLoess, filteredIndex } = data;
      // if regressionResults is not provided, compute it
      const regressionResults = data.regressionResults ?? computeRegression({xData, yData, filteredIndex});
  ...
  useEffect(() => {
    ...
    if (rendered && filteredIndexes && filteredIndexes.length > 0 && chart) {
      const {xData, yData, xVariableName, yVariableName, showRegressionLine, showLoess} = data;
      const regressionResults = computeRegression({xData, yData, filteredIndex: filteredIndexes});
      ...
      if (chart && filteredIndexes.length < xData.length) {
