...
export function getScatterplotChartOption({
  ...
  const createRegressionLine = (results: RegressionResults) => 
    getRegressionLine(results, bounds.minX, bounds.maxX, bounds.minY, bounds.maxY);

  // Calculate regression lines conditionally
  const regressionLines = showRegressionLine && allRegressionResults ? {
    main: createRegressionLine(allRegressionResults),
    selected: selectedRegressionResults && createRegressionLine(selectedRegressionResults),
    unselected: unselectedRegressionResults && createRegressionLine(unselectedRegressionResults)
  } : null;
  ...
  const createLoessSeries = () => showLoess && loessResult ? [
    {
      type: 'line',
